 ...
 * 拖拽事件的事件处理器，用来处理与拖拽有关的一系列浏览器事件。控件是该处理器获知拖拽事件的开始，发生和结束。
 * 
...
SuperMap.Handler.Drag = SuperMap.Class(SuperMap.Handler, {
    ...
    dragstart: function (evt) {
        ...
        if (this.checkModifiers(evt) &&
               (SuperMap.Event.isLeftClick(evt) ||
                SuperMap.Event.isSingleTouch(evt))) {
            this.started = true;
            this.start = evt.xy;
            this.last = evt.xy;
    ...
    addDocumentEvents: function() {
        SuperMap.Element.addClass(document.body, "olDragDown");
    ...
    removeDocumentEvents: function() {
        SuperMap.Element.removeClass(document.body, "olDragDown");
