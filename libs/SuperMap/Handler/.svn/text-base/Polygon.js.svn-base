...
SuperMap.Handler.Polygon = SuperMap.Class(SuperMap.Handler.Path, {
     ...
     * {String} 触发hole digitizing的功能键，允许设置"altKey"、"shiftKey"、"ctrlKey"。
     * 如果没设置，则不会发生hole digitizing，默认值为 null。
     */
    holeModifier: null,
     ...
     * callbacks - {Object} 回调函数对象，关于回调的具体详细说明参见下文。
     当路径绘制完成时，一个对象用done属性值作为一个函数回调。回调得到多边形的几何     
     * 图形的单个参数。如果这个回调对象包含一个point属性，这个函数将会添加发送到每个点，如果这个回调对象包含cancel属性，
     * 当绘制时处理事件是关闭的时候，这个函数将会调用，得到一个几何图形。
