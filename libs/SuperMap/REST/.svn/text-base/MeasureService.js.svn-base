    ...
    processAsync: function(params) {
        ...
        if (me.measureMode == SuperMap.REST.MeasureMode.AREA) {
            if (me.isInTheSameDomain) {
                url = me.url + ((end == "/") ? "area.json?": "/area.json?");
            }
            else {
                url = me.url + ((end == "/") ? "area.jsonp?": "/area.jsonp?");
            }
        } else {
            if (me.isInTheSameDomain) {
                url = me.url + ((end == "/") ? "distance.json?": "/distance.json?");
            }
            else {
                url = me.url + ((end == "/") ? "distance.jsonp?": "/distance.jsonp?");
            }
        }
        ...
        unit = params.unit;    
        urlParameters= "point2Ds=" + SuperMap.Util.toJSON(point2ds) + "&unit=" + unit;
        url += urlParameters;
        me.request({
            url: url,
            method: "GET",
