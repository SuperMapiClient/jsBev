...
/**
 * Class: SuperMap.REST.MTSPPath
 * 多旅行商分析结果路径对象。
 * 该类包含了当前每配送中心所负责的配送目的地。
...
SuperMap.REST.MTSPPath = SuperMap.Class({
    /** 
     * APIProperty: center
     * {Array(<SuperMap.Feature.Vector>)} 本路径对应的配送中心。  
     */    
    center: null,
...
SuperMap.REST.MTSPPath.fromJson = function(jsonObject){
    if (!jsonObject) {
        return;
    }
    var result = new SuperMap.REST.MTSPPath({
    ...
    result.ceter = jsonObject.center;
    return result;
};
