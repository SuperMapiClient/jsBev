 ...
 * label - {String} 可选的标签文本,在浏览器上用canvas渲染器，需要设这fillText或者mozDrawText属性。
 ...
 * labelXOffset - {Number} 标签在x轴方向的偏移量，不支持canvas渲染器。
 * labelYOffset - {Number} 标签在y轴方向的偏移量，不支持canvas渲染器。
